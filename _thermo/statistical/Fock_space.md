...
Where $\alpha_i$ quantify the probability for the particle to be observed as up or down ($\alpha_1\alpha_1^* +\alpha_1\alpha_2^*=1$). The $\alpha_i$ can be function of time if the system evolves through the action of an Hamiltonian operator. The Hilbert space of two identical particles treated together is $\mathcal{H}_2= \mathcal{H}(1) \otimes \mathcal{H}(1)$. A basis for this space is $\lbrace \ket{\phi_1\phi_1}, \ket{\phi_1\phi_2}, \ket{\phi_2\phi_1}, \ket{\phi_2\phi_2} \rbrace$, where we used the shortcut notation
...
Note the close proximity of this formalism with the [quantum harmonic oscillator](../../_quantum/quantum/QHO.md). Creation and anhilation operators can similary be defined in Fock space allowing to move between states with different occupation number. Furthermore, the proper treatment of the continuous space where the basis vector of interests are the space $\ket{x}$, will lead us to define formally the notion of a quantum field. If you want to know more on this topic, a good follow up from this lecture is to look at how the basis vectors $\ket{\phi_i}$ can be constructed concretly in the case of the Schrödinger equation [here](../../_quantum/QFT/SchrodingerI.md), from which it is possible to build a first quantum field [here](../../_quantum/QFT/SchrodingerI.md).
