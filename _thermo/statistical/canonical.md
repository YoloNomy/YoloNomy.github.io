...
Imagine that we have no informations at all about the system, but that the microstates can be distinguished by some property. They could be associated with different observed colors or anything you might want. If we step away for a moment from the isolated case in order to prepare our future discussion, we could imagine that each particle configuration can be associated with a different energy $E_i$ (for an isolated system the total energy is fixed and is the same for each possible microstate).
Let $g_n$ be the **degeneracy** that is the number of possible microstates (particle configurations) amongst the $N$ associated with the same property, like the energy $E_n$. Now the index $n$ goes over all the possible energy values, while $i$ will label the $N$ accessible microstates. For example $g(E_2)=3$ would mean that there exist three different particle configurations in the box with the same energy state $E_2$. In the case of the fair dice analogy, this could mean that amongst our 6 faces labelled by indices $i$, faces could also have a color $C_n=\lbrace \text{blue}, \text{green}...\rbrace$. Multiple faces could have the same colors. For example, $g_2 = g(\text{green})=3$ would mean that three faces amongst the 6 are green.
