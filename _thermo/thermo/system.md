...
The set of all the state variables at a given time, is simply called the **state** of the system. For now, unless otherwise stated, we will assume that the system we consider is described entierly by a single temperature (thermal equilibrium) and a single pressure. Clearly, this is a simplifying assumption, as extended objects can have different temperature/pressure at different point. We'll leave such scenarios for later.
